...
class Rob2Wh:
    ...
    def UKF(self,mu_prev, Sig_prev, ut, zt, marker):
        ...
        # alpha = .5 #?
        # Beta = 2
        k = 4 # no specific values for k and alpha.  I will use the recommended values on the slides
        alpha = .4 #?
        Beta = 2
        ...
        if marker == 0:
            ...
            for i in range(2*n+1):
                term2 = np.array([Xs_x_bar[:,i]]).T-mu_bar
        ...
        Zbar = self.sigma_measurements(Xs_x_bar,Xs_z, marker)
        zhat = (wm@Zbar).T
        zhat[1] = wrap(zhat[1])
        St = np.zeros((2,2))
        Sig_xz = np.zeros((3,2))
    ...
    def sigma_measurements(self, Xbar_x, xs_z, marker):
        ...
        for i in range(15):
            ...
            Z_bar1 = np.array([z_meas[marker],z_meas[marker+3]])
            Z_bar[i,:] = np.array(Z_bar1+np.array([xs_z[:,i]]).T).T
